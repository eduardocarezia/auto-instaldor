...
function parseOptions(argv) {
  const result = {
    ...
    authUrl: process.env.IDEAL_AUTH_URL || "https://hub.automatiza-ai.com",
    scopeProvided: false,
...
Uso:
  ...
  npx ideal-ai-first@latest ideal:instalar --auth-url https://sua-area-de-membros.com [--escopo projeto|global|ambos] [--targets codex,claude,cursor]
...
async function maybeRunInstallWizard(options) {
  ...
  const { rl, close } = prompt;
  try {
    console.log("Assistente de instalacao IDEAL Meta-Squad");
    console.log("Aperte Enter para aceitar as recomendacoes.");
    console.log("");

    if (!options.targetsProvided) {
      options.targets = await askTargets(rl);
    }
    ...
    if (!options.authUrlProvided) {
      options.authUrl = await askText(rl, "Area de membros", options.authUrl);
    }
    ...
    console.log(`  Area de membros: ${options.authUrl}`);
    console.log("");

    const confirmed = await askText(rl, "Confirmar instalacao? [S/n]", "S");
    if (/^n/i.test(confirmed.trim())) {
...
function createPrompt() {
  ...
  if (process.platform !== "win32" && fs.existsSync("/dev/tty")) {
    ...
    return {
      rl: createInterface({ input, output: process.stdout }),
      close: () => input.close(),
    };
  }
...
async function askTargets(rl) {
  ...
  console.log("Ferramentas para instalar:");
  for (const [number, , label] of choices) {
    console.log(`  ${number}. ${label}`);
  }

  while (true) {
    const answer = await askText(rl, "Escolha uma ou mais ferramentas (ex: 1,2 ou codex,claude)", "1,2");
    const targets = parseTargetSelection(answer, choices);
    if (targets.length) return targets;
    console.log("Opcao invalida. Use numeros, nomes ou 'todos'.");
  }
}

function parseTargetSelection(answer, choices) {
  const known = new Map(choices.flatMap(([number, value]) => [[number, value], [value, value]]));
  const normalized = answer.trim().toLowerCase();
  if (["todos", "all", "*"].includes(normalized)) return choices.map(([, value]) => value);

  return normalized
    .split(/[,\s]+/)
    .map((part) => known.get(part))
    .filter(Boolean)
...
async function askScope(rl) {
  console.log("");
  console.log("Escopo da instalacao:");
  console.log("  1. Global - instala uma vez para usar em todos os projetos");
  console.log("  2. Projeto - instala apenas na pasta atual");
  console.log("  3. Ambos - global e projeto atual");

  while (true) {
    const answer = await askText(rl, "Escolha o escopo", "1");
    const scope = parseScopeSelection(answer);
    if (scope) return scope;
    console.log("Opcao invalida. Use 1, 2, 3, global, projeto ou ambos.");
  }
}

function parseScopeSelection(answer) {
  const aliases = {
    1: "global",
    global: "global",
    2: "project",
  ...
  return aliases[answer.trim().toLowerCase()] || "";
}
...
function skillDocument(phase, toolName) {
  ...
  return `---
name: ideal-${phase}
description: ${JSON.stringify(descriptions[phase] || `Use esta skill no ${toolName} para aplicar o Metodo IDEAL.`)}
---
