...
function installClaudeDesktopGlobal(options) {
  writeClaudeDesktopPackages(options, ".claude/desktop-skills");
  syncClaudeDesktopLocalSkills(options);
}

function writeClaudeDesktopPackages(options, root) {
  ...
  zipClaudeDesktopSkills(options, root);
}
...
function zipClaudeDesktopSkills(options, root) {
  const skills = [
...
function syncClaudeDesktopLocalSkills(options) {
  if (!options.claudeDesktopSync) {
    log(options, "skip", "sync local do Claude Desktop desativado");
    return;
  }
  ...
  for (const pluginRoot of pluginRoots) {
    ...
    log(options, "sync", `Claude Desktop local: ${pluginRoot}`);
  }
}
...
function updateClaudeDesktopManifest(options, pluginRoot, artifacts) {
  ...
  for (const artifact of artifacts) {
    ...
    if (existing) {
      ...
      if (String(existing.skillId || "").startsWith("ideal-ai-first-") && existing.description !== artifact.description) {
...
function findClaudeDesktopSkillsPluginRoots() {
  const base = getClaudeDesktopSkillsPluginBase();
  if (!base || !fs.existsSync(base)) return [];

  const roots = [];
  for (const org of listDirectories(base)) {
    for (const account of listDirectories(org)) {
      const manifestPath = path.join(account, "manifest.json");
      const skillsPath = path.join(account, "skills");
      if (fs.existsSync(manifestPath) && fs.existsSync(skillsPath)) {
        roots.push(account);
      }
    }
  }
...
function getClaudeDesktopSkillsPluginBase() {
  const home = os.homedir();
  if (process.platform === "darwin") {
    return path.join(home, "Library", "Application Support", "Claude", "local-agent-mode-sessions", "skills-plugin");
  }
  if (process.platform === "win32") {
    const appData = process.env.APPDATA || path.join(home, "AppData", "Roaming");
    return path.join(appData, "Claude", "local-agent-mode-sessions", "skills-plugin");
  }
  const configHome = process.env.XDG_CONFIG_HOME || path.join(home, ".config");
  return path.join(configHome, "Claude", "local-agent-mode-sessions", "skills-plugin");
}
