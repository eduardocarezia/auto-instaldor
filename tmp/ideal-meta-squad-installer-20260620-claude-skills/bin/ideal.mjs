...
function writeClaudeDesktopPackages(options, root) {
  ...
  for (const phase of IDEAL_COMMANDS) {
    const skillRoot = `${root}/ideal-${phase}`;
    const content = skillDocument(phase, "Claude Desktop/Web");
    writeFile(options, `${skillRoot}/skill.md`, content);
  }

  for (const command of METASQUAD_COMMANDS) {
    const skillRoot = `${root}/metasquad-${command}`;
    const content = metaSquadSkillDocument(command, "Claude Desktop/Web");
    writeFile(options, `${skillRoot}/skill.md`, content);
  }
...
description: ${JSON.stringify(descriptions[command] || `Use esta skill no ${toolName} para aplicar o framework Meta Squad.`)}
---
